...
public:
    ...
    std::string read()
    {
        ...
        std::vector<int> response_args_ints;
        ...
        {
            response_args.push_back(response_data[i]);
        }

        for (unsigned int i = 0; i < response_args.size(); i++)
        {
            std::string val = response_args[i];
            // Convert hex in string form to int
            response_args_ints.push_back(std::stoi(val, 0, 16));
        }

        if (response_type == "*")
        {
            // ACK
            int respondingID = response_args_ints[0];
        ...
        else if (response_type == "!")
        {
            ...
            int NAKCode = response_args_ints[2];
        ...
        else if (response_type == "#")
        {
            ...
            int respondingID = response_args_ints[0];
            // int commandBeingRespondedTo = response_args_ints[1];
            std::vector<int> data(response_args_ints.begin() + 2, response_args_ints.end());
